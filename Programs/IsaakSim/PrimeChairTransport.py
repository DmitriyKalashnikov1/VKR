...
bordur = DynamicCuboid(prim_path="/World/bordur", scale=np.array([2.5, 2, 0.4]), visual_material = omnipbr, position=np.array([3, 0.0, 0.0]), mass=670, physics_material=groundMat)
...
for i in range(25*60):
    ...
    if ((i > 60*5)and(i < 60*8)):
        #velocityes in rad/s
        action = ArticulationAction(joint_velocities=np.array([-2, -2, 7, 7]), joint_indices=np.array([slb,slf,srb,srf]))
        arti_view.apply_action(action)
    
    if ((i > 60*8)and(i < 60*8.5)):
        action = ArticulationAction(joint_velocities=np.array([-2, -2, 2, 2]), joint_indices=np.array([slb,slf,srb,srf]))
        arti_view.apply_action(action)
        action = ArticulationAction(joint_positions=np.array([ 3.66, -3.66]), joint_indices=np.array([nlf, nrf]))
        arti_view.apply_action(action)

    if ((i > 60*8.5)and(i < 60*10)):
        action = ArticulationAction(joint_velocities=np.array([0, 0, 0, 0]), joint_indices=np.array([slb,slf,srb,srf]))
        arti_view.apply_action(action)
        action = ArticulationAction(joint_velocities=np.array([2, 2]), joint_indices=np.array([lf,rf]))
        arti_view.apply_action(action)
    
    if ((i > 60*10)and(i < 60*12.5)):
        action = ArticulationAction(joint_velocities=np.array([-2, 2]), joint_indices=np.array([slb,srb]))
        arti_view.apply_action(action)
        action = ArticulationAction(joint_positions=np.array([ 5.2, -5.2]), joint_indices=np.array([nlf, nrf]))
        arti_view.apply_action(action)

    if ((i > 60*12.5)and(i < 60*15)):
        action = ArticulationAction(joint_velocities=np.array([2, 2]), joint_indices=np.array([lb,rb]))
        arti_view.apply_action(action)
        action = ArticulationAction(joint_positions=np.array([ 0, 0]), joint_indices=np.array([nlb, nrb]))
        arti_view.apply_action(action)
